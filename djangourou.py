...
class Csv(Tools):
    ...
    def start(self):
        ...
        if self.checkCoureurs(self.coureurs):
            self.writeCsv(self.coureurs, self.fickikou)
            print('<{}> OK'.format(self.ficcsv))
            print('<{}> OK a tranférer sur KiKourou'.format(self.fickikou))
            # sauvegarde des fichiers de source vers sv_fic_source
            fic = 'mv {}* {}sv_fic_source'.format(self.ficcsv[:-4], REPWORK)
            sv =  os.system(fic)
        else:
...
class Course(Tools):
    ...
    def start(self):
        for epreuve in self.liste_epreuves():
            ...
            # print('epreuve {} :'.format(self.fic_epreuve.split('/')[-1]))
            disp_epreuve = 'epreuve {} :'.format(self.fic_epreuve.split('/')[-1])
            ...
            if self.check_format(): 
                ...
                with open(self.fic_epreuve, 'w')as out:
                    for c in self.coureurs:
                        out.write('{}\n'.format(c))
                print('probleme avec le fichier {}'.format(self.fic_epreuve))
            ...
            if suite:
                if self.checkCoureurs(self.coureurs):
                    ...
                    print('{} OK'.format(disp_epreuve))
                    self.writeCsv(self.coureurs, dest)
                else:
                    print('{} NOK'.format(disp_epreuve))
                    self.writeCsv(self.coureurs, self.fic_epreuve)
                    for cle, valeur in self.anos.items():
                        print('-->anomalie de type : ', cle)
                        for ano in valeur:
                            print('  --> {}'.format(ano))        
